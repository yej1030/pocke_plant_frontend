...
export default function PlantDetail({ navigation, route }) {
  ...
  useEffect(() => {
    if (route?.params?.hardwareConnected || plant?.macAddress) {
      setIsHardwareConnected(true);
    }
  }, [route?.params?.hardwareConnected, plant?.macAddress]);
  ...
  }, [isHardwareConnected, plant?.macAddress]);
  ...
  const sampledHistory =
    sensorHistory.length > 120
      ? sensorHistory.filter((_, index) => {
          const step =
            Math.ceil(sensorHistory.length / 120);
  ...
  const getChartValues = type => {
    return sampledHistory
      .map(item => Number(item?.[type]))
      .filter(value => Number.isFinite(value));
  };

  const makeSingleChartData = type => {
    const values =
      getChartValues(type);
    ...
    const graphMaxValue =
      Math.max(graphMax - graphMin, 1);

    const data =
      sampledHistory.map((item, index) => {
        const rawValue =
          Number(item?.[type]);
        ...
        const isLast =
          index === sampledHistory.length - 1;

        return {
          value: safeValue - graphMin,
          rawValue: safeValue,
          label:
            isFirst || isLast
              ? formatTimeShort(item?.regDate)
              : '',
        };
      });
  ...
  const latestUpdatedText =
    sensorData?.regDate
      ? formatDateTime(sensorData.regDate)
      : '아직 데이터 없음';
          ...
          <>
            ...
            <View style={styles.statsGrid}>
              {stats.map(item => {
                ...
                return (
                  <TouchableOpacity
                    key={item.label}
                    style={styles.miniSensorCard}
                    activeOpacity={0.85}
                  >
                    <View style={styles.miniSensorHeader}>
                      <Image
                        source={item.icon}
                        style={styles.miniSensorIcon}
                      />

                      <Text style={styles.miniSensorLabel}>
                        {item.label}
                      </Text>
                    </View>

                    <Text style={styles.miniSensorValue}>
                      {item.value}
                      <Text style={styles.miniSensorUnit}>
                        {item.unit}
                      </Text>
                    </Text>

                    <Text style={styles.miniSensorTarget}>
                      적정 {item.target}
                      {item.unit}
                    </Text>
            ...
            <View style={styles.chartsSection}>
              ...
              <Text style={styles.sectionSubText}>
                처음부터 현재까지 전체 {sensorHistory.length}개 데이터
              </Text>

              {chartConfigs.map(config => {
                const chart =
                  makeSingleChartData(config.type);

                const latestValue =
                  sensorData?.[config.type];
                  ...
                  >
                    <View style={styles.chartHeader}>
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={styles.chartSubText}>
                          최저 {formatSensorValue(chart.min, config.type)}
                          {config.unit} · 최고 {formatSensorValue(chart.max, config.type)}
                          {config.unit}
                        </Text>
                      </View>

                      <Text style={styles.chartCurrentValue}>
                        {formatSensorValue(latestValue, config.type)}
                        <Text style={styles.chartCurrentUnit}>
                          {config.unit}
                        </Text>
                      </Text>
                    </View>
                    ...
                    {chart.data.length >= 2 ? (
                      <LineChart
                        ...
                        areaChart
                        curved
                        hideDataPoints={chart.data.length > 20}
                        thickness={3}
                        color={config.color}
                        ...
                        maxValue={chart.graphMaxValue}
                        animateOnDataChange
                        animationDuration={500}
                        pointerConfig={{
                          pointerStripHeight: 130,
                          pointerStripColor: '#A6DDB9',
                          pointerStripWidth: 2,
                            ...
                            return (
                              <View style={styles.pointerLabel}>
                                <Text style={styles.pointerLabelText}>
                                  {formatSensorValue(rawValue, config.type)}
                                  {config.unit}
                                </Text>
                              </View>
                            );
